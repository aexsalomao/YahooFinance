...
let miscMetrics = 
    aaplChain.Quote.ForwardPe,
    aaplChain.Quote.BookValue,
    aaplChain.Quote.MarketCap,
    aaplChain.Quote.SharesOutstanding

// Option contracts
let calls, puts = 
    aaplChain.Options |> Array.collect (fun xs -> xs.Calls),
    aaplChain.Options |> Array.collect (fun xs -> xs.Puts)

let impliedVolOfCheapestCall = 
    calls
    |> Array.sortBy (fun xs -> xs.LastPrice)
    |> Array.head
    |> fun c -> c.ImpliedVolatility

let cheapestInTheMoneyPut = 
    puts
    |> Array.filter (fun xs -> xs.InTheMoney)
    |> Array.sortBy (fun xs -> xs.LastPrice)