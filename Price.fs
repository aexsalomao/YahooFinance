...
module Series = 
    ...
    let private foldQuotesResult (okQueries, failedQueries) (queryResult : Result<Series, string>) = 
        match queryResult with
        | Ok quotes -> ([quotes] @ okQueries, failedQueries)
        | Error e -> (okQueries, [e] @ failedQueries)
    
    let getSeries queries = 
        queries 
        |> getResult
        |> fun queriesResult -> 
            let okQueries, failedQueries = List.fold foldQuotesResult ([], []) queriesResult
            {Data = okQueries ; ErrorLog = failedQueries}
